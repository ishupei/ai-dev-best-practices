...
def main() -> int:
    ...
    parser.add_argument("--scale", type=float, default=3.0, help="Raster scale for PNG")
    ...
    if output_path.suffix.lower() != f".{args.format}":
        print(f"output extension must be .{args.format}", file=sys.stderr)
        return 1
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="tianyin-diagram-") as temp_dir:
            diagrams = render_mermaid_diagrams(source, args.format, Path(temp_dir), args.scale)
            if len(diagrams) != 1:
    ...
    result = {"output": str(output_path), "format": args.format, "bytes": output_path.stat().st_size}
    if args.format == "png":
        result["dimensions"] = png_dimensions(output_path)
